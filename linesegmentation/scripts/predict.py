...
def main():
    ...
    parser.add_argument("--space_height", type=int, default=20,
                        help="Average space between two lines. If set to '0', the value will be calculated automatically ...")
    parser.add_argument("--target_line_height", type=int, default=10,
                        help="Scale the data images so that the space height matches this value (must be the same as in training)")
    ...
    for i_ind, i in tqdm.tqdm(enumerate(lineDetector.detect(gray_file_paths)),total = len(gray_file_paths)):
        pass
